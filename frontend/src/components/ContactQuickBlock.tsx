...
export function ContactQuickBlock({ title = "Hỗ trợ nhanh", variant = "card", className = "" }: Props) {
  ...
  if (variant === "grid") {
    return (
      <div className={`grid gap-4 md:grid-cols-2 ${className}`}>
        <a href={`tel:${contact.hotline}`} className="card flex items-center gap-4 transition hover:-translate-y-1">
  ...
  if (variant === "compact") {
    return (
      <p className={`text-sm text-slate-600 ${className}`}>
        Gọi <a className="font-semibold text-brand-700" href={`tel:${contact.hotline}`}>{contact.hotline}</a>
        {" · "}
        <a className="font-semibold text-brand-700" href={contact.zaloUrl} target="_blank" rel="noreferrer">
          Zalo
        </a>
      </p>
    );
  ...
  return (
    <div className={className}>
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <a className="btn-secondary py-2 text-sm" href={`tel:${contact.hotline}`}>
          <Phone size={16} /> Gọi {contact.hotline}
        </a>
        <a className="btn-secondary py-2 text-sm" href={contact.zaloUrl} target="_blank" rel="noreferrer">
